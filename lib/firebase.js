...
import { getFirestore, doc, setDoc, getDoc, collection, query, where, getDocs, deleteDoc, writeBatch, increment, addDoc, serverTimestamp, orderBy, limit } from "firebase/firestore";
...
export const loginWithGoogle = async () => {
  ...
  // [RBAC] Check existing profile for role, or create with default 'student'
  const userRef = doc(db, 'users', user.uid);
  const userSnap = await getDoc(userRef);
  let userRole = 'student'; // Default role for new users

  if (!userSnap.exists()) {
    // First-time user: create profile with role injection
    ...
    // Returning user: read existing role, update lastLogin
  ...
  // Return user object enriched with role
