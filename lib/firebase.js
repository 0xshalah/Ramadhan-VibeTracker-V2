...
export const updateUserProgress = async (uid, dateId, data) => {
  try {
    ...
    // [DECREE 2] Separate Payload: targetTilawah to profile, activity to daily
    const { targetTilawah, earnedXP, ...dailyData } = data;
    
    // Save daily progress
    await setDoc(docRef, dailyData, { merge: true });
    
    // Aggregate Profile (Target Tilawah & XP)
    const profilePayload = { 
        lastActivity: new Date().toISOString()
    };
    if (targetTilawah !== undefined) profilePayload.targetTilawah = targetTilawah;
    if (earnedXP !== undefined) profilePayload.totalXP = increment(earnedXP);
