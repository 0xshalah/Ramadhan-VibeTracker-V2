      ...
      {students.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {students.map(s => {
             ...
             return (
              <div key={s.uid} className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
                 ...
                 <div className="space-y-3 mt-4">
                    <div className="flex justify-between text-xs font-bold items-center">
                      ...
                      <span className="text-orange-500">{s.streak || 0} Hari</span>
                    </div>
...
function FallbackTeacherError({ error, resetErrorBoundary }: { error: Error, resetErrorBoundary: () => void }) {
  return (
    <div className="p-6 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-3xl border border-red-200 dark:border-red-800">
      <h3 className="font-bold mb-2">Terjadi Kesalahan di Teacher Dashboard</h3>
